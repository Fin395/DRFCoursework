...
class HabitSerializer(serializers.ModelSerializer):
    related_habit = serializers.PrimaryKeyRelatedField(
        queryset=Habit.objects.none(), allow_null=True, required=False
    )
    ...
    def validate(self, data):
        ...
        if current_data.get("is_pleasant"):
            ...
            if current_data.get("related_habit") and current_data.get("award"):
                raise ValidationError(
                    "У полезной привычки не могут быть одновременно связанная привычка и вознаграждение."
                )

            elif not current_data.get("related_habit") and not current_data.get(
                "award"
            ):
                raise ValidationError(
                ...
                not current_data.get("time")
                or not current_data.get("place")
                or not current_data.get("time_to_complete")
            ):
                raise ValidationError(
                    "Для полезной привычки необходимо указать время, место, время на выполнение."
                )
                ...
                if (
                    current_data.get("interval") == 0
                    or current_data.get("interval") > 7
                ):
                    raise ValidationError(
                        "Периодичность выполнения не должна быть равна 0 или более 7."
                    )
